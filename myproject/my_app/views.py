...
def dashboard(request):
    ...
    if low_stock_count > 0:
        if out_of_stock > 0:
            messages.warning(
                request,
                f"Critical Inventory Alert: {low_stock_count} items are below 10 units (including {out_of_stock} out of stock)!"
            )
        else:
            messages.warning(
                request,
                f"Low Stock Alert: {low_stock_count} items are below 10 units!"
            )
...
def inventory_list(request):
    ...
    if low_stock_count > 0:
        messages.warning(
            request,
            f"Attention: {low_stock_count} items in your inventory are running low (below 10 units)!"
        )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if stock.quantity < 10:
                messages.warning(
                    request,
                    f"Note: {stock.name} was added with low stock ({stock.quantity} units)."
                )
            return redirect('add_stock')
    else:
        form = StockForm()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if 0 < stock.quantity < 10:
                messages.warning(
                    request,
                    f"Note: {stock.name} is now in low stock ({stock.quantity} units)."
                )
            elif stock.quantity == 0:
                messages.error(
                    request,
                    f"Alert: {stock.name} is now out of stock!"
                )
...
def stock_in(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Successfully added {transaction.quantity} to {product.name}."
            )
            return redirect('stock_history')
    else:
        form = StockTransactionForm()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if product.quantity >= transaction.quantity:
                ...
                messages.success(
                    request,
                    f"Successfully removed {transaction.quantity} from {product.name}."
                )

                if product.quantity < 10:
                    messages.warning(
                        request,
                        f"Critical Alert: {product.name} has dropped to {product.quantity} units!"
                    )
                ...
                messages.error(
                    request,
                    f"Insufficient stock for {product.name}. Current quantity: {product.quantity}"
                )
    else:
        form = StockTransactionForm()
